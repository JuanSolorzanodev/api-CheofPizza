...
{
    /**
     * Devuelve una lista unificada y paginada de movimientos financieros.
     *
     * La fecha efectiva depende del origen:
     *
     * - cash: transición del pedido al estado delivered;
     * - transfer: reviewed_at, o submitted_at mientras esté pendiente;
     * - paypal: timestamp correspondiente al estado del pago.
     *
     * @param array{
     *     method?: string|null,
     *     status?: string|null,
    ...
    public function paginate(
        ...
        $query = DB::query()
            ->fromSub(
                $this->unifiedTransactions($range),
        ...
        if (filled($filters['search'] ?? null)) {
            $search = '%' . $this->escapeLike(
                (string) $filters['search'],
            ) . '%';

            $query->where(
                static function (Builder $builder) use ($search): void {
                    $builder
                        ->where(
    ...
    private function unifiedTransactions(
        ...
        return $this->cashTransactions($range)
            ->unionAll(
                $this->transferTransactions($range),
            )
            ->unionAll(
                $this->paypalTransactions($range),
            );
    }

    /**
     * Un pedido en efectivo se reconoce una sola vez: en su primera
     * transición a delivered dentro del periodo.
     */
    private function cashTransactions(
        ...
        $deliveredChanges = DB::table(
            ...
            ->selectRaw(
                ...
                MIN(order_status_changes.changed_at) AS effective_at
                '
            )
            ->groupBy(
                'order_status_changes.order_id',
        ...
        return DB::table('orders')
            ...
            ->selectRaw(
                '
                CONCAT("cash:", orders.id) AS transaction_key,
                "order" AS source,
                ...
                TRIM(CONCAT(users.first_name, " ", users.last_name)) AS customer_name,
    ...
    /**
     * Se muestra el último comprobante de cada pedido para evitar que un
     * reintento rechazado y luego aprobado aparezca como dos saldos activos.
     */
    private function transferTransactions(
        ...
        return DB::table('payment_receipts')
            ...
            ->whereBetween(
                DB::raw(
                    'COALESCE(payment_receipts.reviewed_at, payment_receipts.submitted_at)'
                ),
                [
                    $range->from,
                    $range->to,
            ...
            ->selectRaw(
                '
                CONCAT("transfer:", payment_receipts.id) AS transaction_key,
                ...
                NULLIF(
                    TRIM(CONCAT(reviewers.first_name, " ", reviewers.last_name)),
                    ""
                ) AS reviewed_by,
                NULL AS failure_code
                '
    ...
    private function paypalTransactions(
        ...
        $effectiveAt = '
            CASE payments.status
                WHEN "refunded" THEN COALESCE(payments.refunded_at, payments.updated_at)
                WHEN "partially_refunded" THEN COALESCE(payments.refunded_at, payments.updated_at)
                WHEN "completed" THEN COALESCE(payments.paid_at, payments.updated_at)
                WHEN "approved" THEN COALESCE(payments.approved_at, payments.updated_at)
                WHEN "failed" THEN COALESCE(payments.failed_at, payments.updated_at)
                WHEN "denied" THEN COALESCE(payments.failed_at, payments.updated_at)
                WHEN "cancelled" THEN COALESCE(payments.cancelled_at, payments.updated_at)
        ...
        return DB::table('payments')
            ...
            ->selectRaw(
                '
                CONCAT("paypal:", payments.id) AS transaction_key,
                "payment" AS source,
